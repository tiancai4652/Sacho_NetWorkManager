...
namespace WinLib.Network
{
    ...
    public class MyNetworkInterface
    {
        ...
        public static MyNetworkInterface Loopback = new MyNetworkInterface();
        ...
        public static ConcurrentDictionary<Guid, MyNetworkInterface> GetAll(Action<string> UpdateStatus = null)
        {
            ConcurrentDictionary<Guid, MyNetworkInterface> NICs = new ConcurrentDictionary<Guid, MyNetworkInterface>();
            try
            {
                ...
                // Loopback
                MyNetworkInterface.Loopback.Name = "Loopback Interface";
                MyNetworkInterface.Loopback.Index = 1;
                if (Environment.OSVersion.Version.CompareTo(new Version("6.0")) > -1)
                {
                    ...
                    MyNetworkInterface.Loopback.networkInterface = loopbackNic;
                    MyNetworkInterface.Loopback.Guid = new Guid(loopbackNic.Id);
                    MyNetworkInterface.Loopback.Name = loopbackNic.Name;
                    MyNetworkInterface.Loopback.Description = loopbackNic.Description;
                    MyNetworkInterface.Loopback.Type = (AdapterType)loopbackNic.NetworkInterfaceType;
                    MyNetworkInterface.Loopback.Mac = BitConverter.ToString(loopbackNic.GetPhysicalAddress().GetAddressBytes().ToArray()).Replace('-', ':');
                    MyNetworkInterface.Loopback.IPv4BytesReceived = loopbackNic.GetIPv4Statistics().BytesReceived;
                    MyNetworkInterface.Loopback.IPv4BytesSent = loopbackNic.GetIPv4Statistics().BytesSent;
                    MyNetworkInterface.Loopback.IPv4Enabled = loopbackNic.Supports(NetworkInterfaceComponent.IPv4);
                    MyNetworkInterface.Loopback.IPv6Enabled = loopbackNic.Supports(NetworkInterfaceComponent.IPv6);
                    if (Environment.OSVersion.Version.CompareTo(new Version("6.0")) < 0)
                        MyNetworkInterface.Loopback.IPv6Enabled = false;
                    if (loopbackNic.GetIPProperties().GetIPv4Properties() != null)
                        MyNetworkInterface.Loopback.Index = loopbackNic.GetIPProperties().GetIPv4Properties().Index;
                    else if (loopbackNic.GetIPProperties().GetIPv6Properties() != null)
                        MyNetworkInterface.Loopback.Index = loopbackNic.GetIPProperties().GetIPv6Properties().Index;
                    foreach (string line in ipv4Mtus)
                        if (Regex.IsMatch(line, @"\s*" + loopbackNic.Name + @"\s*$"))
                            MyNetworkInterface.Loopback.IPv4Mtu = uint.Parse(Regex.Replace(line, @"^\s*(\d+)\s+.+$", "$1"));
                    foreach (string line in ipv6Mtus)
                        if (Regex.IsMatch(line, @"\s*" + loopbackNic.Name + @"\s*$"))
                            MyNetworkInterface.Loopback.IPv6Mtu = uint.Parse(Regex.Replace(line, @"^\s*(\d+)\s+.+$", "$1"));
                    foreach (IPAddress ip in loopbackNic.GetIPProperties().DnsAddresses)
                    {
                        if (ip.AddressFamily == AddressFamily.InterNetwork)
                            MyNetworkInterface.Loopback.IPv4DnsServer.Add(ip.ToString());
                        else if (ip.AddressFamily == AddressFamily.InterNetworkV6)
                            MyNetworkInterface.Loopback.IPv6DnsServer.Add(ip.ToString());
                    }
                    ...
                    foreach (string line in netsh)
                        ...
                        {
                            MyNetworkInterface.Loopback.InterfaceMetric = ushort.Parse(Regex.Replace(line, @"^\s*InterfaceMetric:\s*(\d*)\s*$", "$1"));
                        }
                }
                    ...
                    if (nic.Supports(NetworkInterfaceComponent.IPv4) ||
                        ...
                        if (nic.OperationalStatus == OperationalStatus.Up)
                            ...
                            //if (nic.GetPhysicalAddress().GetAddressBytes().Count() >= 6) // not all interfaces are ethernet emulated
                            {
                                ...
                                NICs.TryAdd(guid, new MyNetworkInterface());
